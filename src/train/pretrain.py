...
def train_ddp(mask_fractions, epochs_for_training, exp_label, mode="pretrain"):
    ...
    find_unused_parameters = (TRANSFORMER_MODEL_NAME == "Bert") or (TRANSFORMER_MODEL_NAME == "Performer" and FINETUNE_TO_RECONSTRUCT_EXPR_OF_ALL_GENES)
    ddp_model = DDP(model, device_ids=[rank], find_unused_parameters=find_unused_parameters)
    params_to_update = []
    if FINETUNE_TO_RECONSTRUCT_EXPR_OF_ALL_GENES:
        print("FINETUNE_TO_RECONSTRUCT_EXPR_OF_ALL_GENES")
        ...
        if PERFORMER_NET_LAST_LAYER_REQUIRES_GRAD:
            print("PERFORMER_NET_LAST_LAYER_REQUIRES_GRAD == True")
    ...
    if TRANSFORMER_MODEL_NAME != "Performer" or FINETUNE_TO_RECONSTRUCT_EXPR_OF_ALL_GENES:
        if OPTIMIZER == "AdamW":
            print("use AdamW optimizer!")
            optimizer = optim.AdamW(params_to_update, lr=MAX_LR/5.0, weight_decay=ADAMW_WEIGHT_DECAY)
        if OPTIMIZER == "Adam":
            print("use Adam optimizer!")
            optimizer = optim.Adam(params_to_update, lr=MAX_LR/5.0)
    # as performer has reversible, so it can't use params_to_update
    else:
        if OPTIMIZER == "AdamW":
            print("use AdamW optimizer!")
            optimizer = optim.AdamW(ddp_model.parameters(), lr=MAX_LR/5.0, weight_decay=ADAMW_WEIGHT_DECAY)
        if OPTIMIZER == "Adam":
            print("use Adam optimizer!")
            optimizer = optim.Adam(ddp_model.parameters(), lr=MAX_LR/5.0)        
    if LOSS_FN == "MSE":
        loss_fn = nn.MSELoss()
    if SCHEDULER == "CyclicLR":
        scheduler = CyclicLR(optimizer, base_lr=BASE_LR, max_lr=MAX_LR, step_size_up=STEP_SIZE_UP, gamma=0.8, cycle_momentum=False, mode='triangular2',  last_epoch=-1)
    elif SCHEDULER == "OneCycleLR":
        print("Use SCHEDULER of OneCycleLR")
        scheduler = OneCycleLR(optimizer, max_lr=MAX_LR, pct_start=ONE_CYCLE_LR_PCT_START, div_factor=ONE_CYCLE_LR_DIV_FACTOR, total_steps=ONE_CYCLE_LR_TOTAL_STEPS, epochs=ONE_CYCLE_LR_EPOCHS)
    # Training loop
    model_loaded = False
    for epoch in epochs_for_training:
        if epoch == 0 and USE_PRETRAIN_MODEL_FOR_FINETUNE and FINETUNE_TO_RECONSTRUCT_EXPR_OF_ALL_GENES:
        ...
        if epochs_for_training[0] != 0 and model_loaded == False:
            ...
            if epoch < EPOCH_TO_HAVE_MANUAL_LR:
                scheduler.step()
            model_loaded = True
        ...
        for dataset_idx in range(0, total_number_of_datasets):
            ...
            if DATASET_TO_GET_FOR_MIXED_DATASET != None:
                dataset = get_mixed_dataset(chunk_idx=dataset_idx, mask_fraction=mask_fraction, dataset_to_get=DATASET_TO_GET_FOR_MIXED_DATASET)
            else:
            ...
            train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=False, sampler=train_sampler, num_workers=12, pin_memory=True, worker_init_fn=worker_init_fn, persistent_workers=True)
            val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE, shuffle=False, sampler=val_sampler, num_workers=12, pin_memory=True, worker_init_fn=worker_init_fn, persistent_workers=True)
...
def main():
    ...
    train_ddp(MASK_FRACTIONS, epochs_for_training, args.exp_label)
